...
export async function signUp(req, res) {
    ...
    try {
        const existingUser = await User.findOne({ email });
        if (existingUser) return res.status(404).send({ message: 'User already exist' });

        if (password !== confirmPassword) return res.status(404).send({ message: 'Passwords do not match' });
